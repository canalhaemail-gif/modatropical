...
$currentCategorySlug = $currentCategorySlug ?? null;
$catalogSearchAction = $catalogSearchAction ?? app_url('busca.php');
$catalogSearchValue = $catalogSearchValue ?? trim((string) ($_GET['q'] ?? ''));
$catalogSearchMode = $catalogSearchMode ?? 'global';
$favoriteCount = $favoriteCount ?? 0;
$headerCartCount = (int) ($cartCount ?? 0);
$currentScript = basename((string) ($_SERVER['SCRIPT_NAME'] ?? ''));
$showBackButton = $showBackButton ?? ($currentScript === 'produto.php');
$customerFirstName = 'Cliente';
$notificationReturnTo = (string) ($_SERVER['REQUEST_URI'] ?? 'notificacoes.php');
$showHeaderCart = storefront_customer_can_use_cart($currentCustomer ?? null);
$customerNotificationsModal = $currentCustomer
    ? fetch_customer_notifications((int) ($currentCustomer['id'] ?? 0), 100)
    : [];

if (!empty($currentCustomer['nome'])) {
    $customerNameParts = preg_split('/\s+/u', trim((string) $currentCustomer['nome']), -1, PREG_SPLIT_NO_EMPTY) ?: [];
    $customerFirstName = (string) ($customerNameParts[0] ?? 'Cliente');
}
?>
<header class="storefront-header">
    <div class="storefront-header__main<?= $showHeaderCart ? ' storefront-header__main--with-cart' : ''; ?>">
        <div class="storefront-brand-stack">
            <a class="storefront-brand" href="<?= e(app_url()); ?>" data-instant-nav>
                <span class="storefront-brand__mark">
                    <?php if (!empty($storefrontBrandLogoUrl)): ?>
                        <img
                            src="<?= e($storefrontBrandLogoRenderUrl ?? $storefrontBrandLogoUrl); ?>"
                            alt="Logo da loja"
                            loading="eager"
                            decoding="async"
                            fetchpriority="high"
                        >
                    <?php else: ?>
                        <span><?= e(substr($storeSettings['nome_estabelecimento'] ?? 'L', 0, 1)); ?></span>
                    <?php endif; ?>
                </span>
            </a>
        ...
        <form
            ...
            method="get"
            action="<?= e($catalogSearchAction); ?>"
            data-catalog-search-form
            data-catalog-search-mode="<?= e($catalogSearchMode); ?>"
        >
            <label class="storefront-search__label" for="catalog-search">Pesquisar produto</label>
            <input
                id="catalog-search"
                type="search"
                name="q"
        ...
        <div
            class="storefront-utility<?= $showHeaderCart ? ' storefront-utility--with-cart' : ''; ?>"
            <?= $currentCustomer ? ' data-utility-menu data-notifications-feed-url="' . e(app_url('notificacoes-widget.php')) . '"' : ''; ?>
        >
            <?php if ($currentCustomer): ?>
                <button
                    class="storefront-utility-menu__toggle"
                    type="button"
                    aria-label="Abrir menu da conta"
                    aria-expanded="false"
                    aria-controls="storefront-utility-panel"
                    data-utility-toggle
                >
                    <span class="storefront-utility-menu__line"></span>
                    <span class="storefront-utility-menu__line"></span>
                    <span class="storefront-utility-menu__line"></span>
                    <?php if (!empty($customerNotificationUnreadCount)): ?>
                        <span class="storefront-notification-menu__count storefront-utility-menu__count" data-utility-menu-count>
                            <?= e((string) $customerNotificationUnreadCount); ?>
                        </span>
                    <?php endif; ?>
                </button>

                <?php if ($showHeaderCart): ?>
                    <a
                        class="floating-cart floating-cart--utility"
                        data-quantity="<?= e((string) $headerCartCount); ?>"
                        href="<?= e(app_url('carrinho.php')); ?>"
                        aria-label="Abrir carrinho"
                        title="Carrinho"
                    >
                        <span class="floating-cart__icon" aria-hidden="true">
                            <svg viewBox="0 0 24.38 30.52" role="presentation" focusable="false">
                                <path d="M24.38 26.45L22.62 6.66a.75.75 0 0 0-.76-.69h-3.7a6 6 0 0 0-12 0H2.51a.76.76 0 0 0-.76.69L0 26.45v.07a4.29 4.29 0 0 0 4.52 4h15.34a4.29 4.29 0 0 0 4.52-4.07ZM12.19 1.52a4.47 4.47 0 0 1 4.46 4.45H7.73a4.47 4.47 0 0 1 4.46-4.45Zm7.67 27.48H4.51a2.79 2.79 0 0 1-3-2.45L3.21 7.49h3V10.2a.76.76 0 0 0 1.52 0V7.49h8.92V10.2a.76.76 0 0 0 1.52 0V7.49h3l1.69 19.06a2.79 2.79 0 0 1-3 2.45Z"></path>
                            </svg>
                        </span>
                        <span class="floating-cart__count" data-global-cart-count><?= e((string) $headerCartCount); ?></span>
                    </a>
                <?php endif; ?>

                <div class="storefront-utility-menu__panel" id="storefront-utility-panel" data-utility-panel>
                    <div class="storefront-utility-menu__view storefront-utility-menu__view--main" data-utility-main-view>
                        <div class="storefront-utility-menu__intro">
                            <span class="storefront-utility-menu__welcome">Ola, <?= e($customerFirstName); ?></span>

                            <button
                                class="storefront-notification-menu__toggle storefront-notification-menu__toggle--inline"
                                type="button"
                                aria-label="Abrir notificacoes"
                                data-utility-notifications-open
                            >
                                <svg viewBox="0 0 24 24" aria-hidden="true">
                                    <path d="M12 3a4 4 0 0 0-4 4v1.2c0 .8-.24 1.58-.69 2.23L5.7 12.8A2 2 0 0 0 7.36 16h9.28a2 2 0 0 0 1.66-3.2l-1.61-2.37A4 4 0 0 1 16 8.2V7a4 4 0 0 0-4-4Zm0 18a2.5 2.5 0 0 0 2.45-2h-4.9A2.5 2.5 0 0 0 12 21Z"></path>
                                </svg>
                                <?php if (!empty($customerNotificationUnreadCount)): ?>
                                    <span class="storefront-notification-menu__count" data-notification-count><?= e((string) $customerNotificationUnreadCount); ?></span>
                                <?php endif; ?>
                            </button>
                        </div>

                        <a class="storefront-utility__link" href="<?= e(app_url('meus-pedidos.php')); ?>">Meus pedidos</a>
                        <a class="storefront-utility__link" href="<?= e(app_url('itens-salvos.php')); ?>">
                            Itens salvos<?= !empty($favoriteCount) ? ' (' . e((string) $favoriteCount) . ')' : ''; ?>
                        </a>
                        <a class="storefront-utility__link" href="<?= e(app_url('meus-cupons.php')); ?>" data-instant-nav>Meus cupons</a>
                        <a class="storefront-utility__link" href="<?= e(app_url('notificacoes.php')); ?>" data-notifications-modal-open>Mensagens</a>
                        <a class="storefront-utility__link" href="<?= e(app_url('minha-conta.php')); ?>">Minha conta</a>
                        <a class="storefront-utility__cta" href="<?= e(app_url('sair.php')); ?>">Sair</a>
                    </div>

                    <div class="storefront-utility-menu__view storefront-utility-menu__view--notifications" data-utility-notifications-view hidden>
                        <div class="storefront-utility-menu__notifications-top">
                            <button class="storefront-utility-menu__back" type="button" data-utility-notifications-back>
                                Voltar
                            </button>
                            <strong>Notificacoes</strong>
                            <button class="storefront-utility-menu__expand" type="button" data-notifications-modal-open>
                                Janela
                            </button>
                        </div>

                        <div class="storefront-utility-menu__notifications-body" data-live-notifications>
                            <?= render_customer_notifications_widget($customerNotifications, (int) $customerNotificationUnreadCount, $notificationReturnTo); ?>
                        </div>
                    </div>
                </div>

                <div class="storefront-notifications-modal" data-notifications-modal hidden>
                    <div class="storefront-notifications-modal__backdrop" data-notifications-modal-close></div>

                    <div
                        class="storefront-notifications-modal__dialog"
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="storefront-notifications-modal-title"
                    >
                        <div class="storefront-notifications-modal__header">
                            <div class="storefront-notifications-modal__copy">
                                <span class="storefront-toolbar__eyebrow">central</span>
                                <strong id="storefront-notifications-modal-title">Notificacoes</strong>
                                <p>Pedidos, cupons e mensagens importantes aparecem aqui.</p>
                            </div>

                            <button class="storefront-notifications-modal__close" type="button" data-notifications-modal-close>
                                Fechar
                            </button>
                        </div>

                        <div
                            class="storefront-notifications-modal__body"
                            data-notifications-modal-body
                            data-notifications-modal-feed-url="<?= e(app_url('notificacoes-widget.php?limit=100')); ?>"
                        >
                            <?= render_customer_notifications_widget($customerNotificationsModal, (int) $customerNotificationUnreadCount, $notificationReturnTo); ?>
                        </div>

                        <div class="storefront-notifications-modal__footer">
                            <a class="btn btn--light" href="<?= e(app_url('notificacoes.php')); ?>">Abrir pagina completa</a>
                        </div>
                    </div>
                </div>
            <?php else: ?>
                <a class="storefront-utility__auth-button" href="<?= e(app_url('entrar.php')); ?>" aria-label="Entrar">
                    <span class="storefront-utility__auth-text">Entrar</span>
    ...
    <?php if ($visibleCategories): ?>
        <div class="storefront-nav-wrap" id="categorias">
            <nav class="storefront-nav" aria-label="Categorias da vitrine">
                <?php foreach ($visibleCategories as $navCategory): ?>
                    <a
                        class="storefront-nav__link<?= $currentCategorySlug === ($navCategory['slug'] ?? null) ? ' is-current' : ''; ?>"
                        href="<?= e(storefront_category_url((string) $navCategory['slug'])); ?>"
                        data-instant-nav
                    >
                        <span class="storefront-nav__label"><?= e($navCategory['nome']); ?></span>
                        <span class="storefront-nav__stars-shell" aria-hidden="true">
                            <span class="storefront-nav__stars"></span>
                        </span>
                        <span class="storefront-nav__glow" aria-hidden="true">
                            <span class="storefront-nav__circle"></span>
                            <span class="storefront-nav__circle"></span>
                        </span>
                    </a>
                <?php endforeach; ?>
