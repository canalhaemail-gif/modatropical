...
if (is_post()) {
    ...
    $emailConfirmation = normalize_email((string) posted_value('email_confirmation'));
    ...
    $address = trim(implode(', ', array_values(array_filter($addressParts, static fn(string $part): bool => $part !== ''))));

    if ($name === '' || $email === '' || $emailConfirmation === '' || $phone === '' || $cep === '' || $street === '' || $district === '' || $number === '' || $city === '' || $state === '' || $cpf === '' || $birthDate === '' || $password === '') {
        set_flash('error', 'Preencha todos os campos obrigatorios.');
        redirect('cadastro.php');
    }
    ...
    if ($email !== $emailConfirmation) {
        set_flash('error', 'A confirmacao do email nao confere.');
        redirect('cadastro.php');
    }
    ...
    if (customer_cpf_exists($cpf)) {
        set_flash('error', 'Ja existe uma conta cadastrada com este CPF.');
        redirect('cadastro.php');
    }

    $customerId = register_customer($name, $email, $phone, $cep, $address, $cpf, $birthDate, $password);
    ...
    $verificationResult = request_customer_email_verification($email);

    if (!empty($verificationResult['delivered'])) {
        set_flash('success', 'Conta criada. Enviamos um codigo e um link de confirmacao para seu email.');
    } elseif (!empty($verificationResult['logged'])) {
        $_SESSION['verification_mail_log_path'] = (string) ($verificationResult['log_path'] ?? '');
        set_flash('success', 'Conta criada. No localhost, o email de confirmacao foi salvo localmente para teste.');
    } else {
        set_flash('error', 'Conta criada, mas nao foi possivel enviar a confirmacao agora. Solicite um novo envio na tela seguinte.');
    }
...
$signupBrandPath = preferred_critical_image_render_url(
...
<main class="page-shell page-shell--signup">
    ...
    <section class="auth-layout auth-layout--signup">
        <article class="auth-form-card auth-form-card--signup">
            <?php
            $socialMode = 'login';
            $socialClusterLabel = 'ou comece com';
                ...
                <section class="signup-section">
                    ...
                    <div class="signup-grid signup-grid--double">
                        <div class="form-row signup-grid__wide">
                            ...
                            <input id="nome" name="nome" type="text" required value="<?= e((string) posted_value('nome')); ?>">
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="cpf">CPF</label>
                            <input id="cpf" name="cpf" type="text" required inputmode="numeric" maxlength="14" placeholder="000.000.000-00" value="<?= e((string) posted_value('cpf')); ?>">
                            <small class="form-row__hint" data-cpf-hint></small>
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="data_nascimento">Data de nascimento</label>
                            <input id="data_nascimento" name="data_nascimento" type="date" required value="<?= e((string) posted_value('data_nascimento')); ?>" data-birthdate-input>
                            <small class="form-row__hint" data-birthdate-hint></small>
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="telefone">Telefone</label>
                            <input id="telefone" name="telefone" type="text" required inputmode="numeric" placeholder="(11) 99999-8888" value="<?= e((string) posted_value('telefone')); ?>">
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="email">Email</label>
                            <input id="email" name="email" type="email" required autocomplete="email" value="<?= e((string) posted_value('email')); ?>" data-email-input>
                            <small class="form-row__hint" data-email-hint></small>
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="email_confirmation">Confirmacao de email</label>
                            <input id="email_confirmation" name="email_confirmation" type="email" required autocomplete="email" value="<?= e((string) posted_value('email_confirmation')); ?>" data-email-confirmation>
                            <small class="form-row__hint" data-email-confirmation-hint>Repita o email para evitar erros de cadastro.</small>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="signup-grid signup-grid--double">
                        <div class="form-row">
                            ...
                            <input
                                ...
                                value="<?= e((string) posted_value('cep')); ?>"
                                data-cep-input
                            >
                        </div>

                        <div class="signup-cep-status" data-cep-status aria-live="polite"></div>
                        ...
                        <div class="form-row signup-grid__wide">
                            ...
                            <input
                                ...
                                value="<?= e((string) posted_value('rua')); ?>"
                                data-address-street
                            >
                        </div>
                        ...
                        <div class="form-row signup-grid__wide">
                            ...
                            <input
                                ...
                                required
                                placeholder="Bairro"
                                value="<?= e((string) posted_value('bairro')); ?>"
                        ...
                        <div class="form-row">
                            ...
                            <input id="numero" name="numero" type="text" required placeholder="123" value="<?= e((string) posted_value('numero')); ?>">
                        </div>

                        <div class="form-row">
                            <label for="complemento">Complemento</label>
                            <input id="complemento" name="complemento" type="text" placeholder="Apto, bloco, referencia" value="<?= e((string) posted_value('complemento')); ?>">
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <input
                                ...
                                required
                                placeholder="Cidade"
                                value="<?= e((string) posted_value('cidade')); ?>"
                                data-address-city
                            >
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <input
                                ...
                                maxlength="2"
                                placeholder="UF"
                                value="<?= e((string) posted_value('uf')); ?>"
                                data-address-state
                            >
                        </div>
                    </div>
                ...
                <section class="signup-section">
                    ...
                    <div class="signup-grid signup-grid--double">
                        <div class="form-row signup-grid__wide">
                            ...
                            <input id="password" name="password" type="password" required autocomplete="new-password" data-password-input>
                            <small class="form-row__hint" data-password-hint>Use pelo menos 8 caracteres, uma letra maiuscula e um caractere especial.</small>
                        </div>

                        <div class="form-row signup-grid__wide">
                            <label for="password_confirmation">Confirmar senha</label>
                            <input id="password_confirmation" name="password_confirmation" type="password" required autocomplete="new-password" data-password-confirmation>
                            <small class="form-row__hint" data-password-confirmation-hint>Repita a senha exatamente como acima.</small>
                        </div>
                    </div>
                </section>

                <button class="btn btn--primary auth-form-card__submit" type="submit">Criar minha conta</button>
            </form>
        </article>
    </section>
</main>
